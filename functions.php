...
*********************************************************/
 $cabin = _x( 'on', 'Cabin font: on or off', 'insightcustom' );
 $font_families = array();
 if ( 'off' !== $cabin ) {
	 $font_families[] = 'Cabin:400,500,700';
 }
 if ( in_array( 'on', array($cabin) ) ) {
	 $query_args = array(
...
function insightcustom_scripts() {
	...
	wp_register_style('grid-gallery', get_stylesheet_directory_uri() . '/css/grid-gallery.min.css');
	wp_enqueue_script( 'insightcustom-lightbox', get_template_directory_uri() . '/js/grid-gallery.js', array(), '20151215', true );
	wp_register_style('perfundo', get_stylesheet_directory_uri() . '/css/perfundo.min.css');
